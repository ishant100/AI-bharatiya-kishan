...
export default function Weather() {
  ...
  // units helpers
  const toTemp = (c) => (units === "metric" ? c : (c * 9) / 5 + 32);
  const toWind = (k) => (units === "metric" ? k : k * 0.621371);
        ...
        <Card className={panelClass}>
          <CardContent className="p-4 space-y-4">
              ...
              <select
                ...
                onChange={(e) => {
                  const v = e.target.value;
                                      const presets = {                    general: { rainMM: 10, windKMH: 20, heatC: 38, coldC: 8 },
            ...
            <div className="grid sm:grid-cols-4 gap-3 text-sm">
              {[
                { label: "Rain ≥ (mm)", key: "rainMM", help: "Alert if daily rain meets/exceeds this." },
                { label: "Wind ≥ (km/h)", key: "windKMH", help: "Avoid spraying when stronger than this." },
                { label: "Heat ≥ (°C)", key: "heatC", help: "Warn for heat stress." },
                { label: "Cold ≤ (°C)", key: "coldC", help: "Warn for cold stress." },
              ].map((f) => (
                <label key={f.key} className="p-3 rounded-2xl bg-white/30">
                  ...
                  <input
                    ...
                    className="w-full mt-1 bg-white/90 text-slate-900 rounded"
                                          value={thresholds[f.key]}                    onChange={(e) =>
